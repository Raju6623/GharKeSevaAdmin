...
const SettlementsTab = ({ vendors = [] }) => {
    ...
    const filteredVendors = vendors.filter(v =>
        v.customUserId?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    return (
        <div className="space-y-8 animate-in slide-in-from-bottom-2 duration-500">
            ...
            {/* Filters & Search */}
            <div className="flex flex-col lg:flex-row items-center justify-between gap-6 bg-white p-4 rounded-[2.5rem] border border-slate-100 shadow-sm">
                <div className="flex flex-wrap items-center gap-2 overflow-x-auto no-scrollbar pb-1">
                    {['PENDING', 'PAID', 'ALL', 'WALLETS'].map(status => (
                        <button
                            key={status}
                        ...
                        >
                            {status === 'WALLETS' ? 'Vendor Wallets' : status}
                        </button>
                    ))}
                </div>
                ...
                <div className="relative w-full lg:w-80">
                    ...
                    <input
                        ...
                        className="w-full pl-12 pr-4 py-3 bg-slate-50 border-none rounded-xl text-sm font-bold text-slate-700 focus:ring-2 focus:ring-indigo-100 outline-none"
                        value={searchTerm}
                ...
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-4">
                        ...
                        <h3 className="text-lg font-black text-slate-800 tracking-tight uppercase">
                            {filter === 'WALLETS' ? 'Vendor Wallet Balances' : 'Withdrawal Requests'}
                        </h3>
                    </div>
                    <span className="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em]">
                        {filter === 'WALLETS' ? filteredVendors.length : filteredWithdrawals.length} Total
                    </span>
                </div>

                {loading && filter !== 'WALLETS' ? (
                            ...
                            <div key={vendor._id} className="p-6 bg-white rounded-3xl border border-slate-100 hover:border-indigo-100 transition-colors flex items-center justify-between gap-4 shadow-sm">
                                <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 rounded-xl bg-slate-100 overflow-hidden border border-slate-200">
                                        <img
                                            src={vendor.vendorPhoto || "https://placehold.co/100"}
                                            alt=""
                                    ...
                                    <div>
                                        <p className="text-sm font-black text-slate-800">{vendor.firstName} {vendor.lastName}</p>
                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{vendor.customUserId} • {vendor.vendorCategory}</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className={`text-lg font-black tracking-tight ${vendor.walletBalance < 0 ? 'text-rose-600' : 'text-emerald-600'}`}>
                                        ₹{(vendor.walletBalance || 0).toLocaleString('en-IN')}
                                    </p>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                                        {vendor.walletBalance < 0 ? 'Commission Debt' : 'Payable Balance'}
                                    </p>
                                </div>
                            </div>
                        )) : (
                            <div className="py-20 text-center text-slate-300 font-bold uppercase tracking-widest text-xs">No matching vendors found</div>
                        )}
                    </div>
                    ...
                    <div className="py-20 flex flex-col items-center justify-center text-slate-300">
                        ...
                        <p className="text-sm font-black uppercase tracking-widest">No matching {filter.toLowerCase()} requests</p>
                        <p className="text-xs font-medium mt-1">Try a different search term</p>
                    </div>
                )}
            </div>
